...
|---|---|---|
| `LUH-A602S.yaml` | `src/tests/api/vesynchumidifier/LUH-A602S-WUS.yaml` | `device_on_warm_mist_level2`: synthesized from pyvesync HUMIDIFIER_DETAILS and device_map.py LUH-A602S HumidifierMap entry (class VeSyncHumid200300S, mist_levels=range(1,10), warm_mist_levels=[0,1,2,3], features=[WARM_MIST,AUTO_STOP]). Note: upstream fixture only covers request payloads; response shapes were synthesized from Classic 300S (same class) with warm fields populated per OasisMist 450S precedent. CRITICAL NOTE: pyvesync PR #505 (open as of 2026-04-27) reports LUH-A602S-WEU needs `mode:"humidity"` instead of `mode:"auto"` for auto mode; the canonical upstream fixture uses `mode:"auto"` which this file follows. See driver CROSS-CHECK comments. |
| `LUH-D301S.yaml` | `src/tests/api/vesynchumidifier/Dual200S.yaml` | `device_on_manual_canonical`: sourced from pyvesync call_json_humidifiers.py HUMIDIFIER_DETAILS["Dual200S"] with HumidifierDefaults constants resolved at commit c98729c. Key difference from Classic 300S: mist range 1-2 only (device_map.py LUH-D301S mist_levels=list(range(1,3))); modes auto+manual only (no sleep); features=[AUTO_STOP] (no NIGHTLIGHT, no WARM_MIST). night_light_brightness is present in the canonical response shape (inherited from VeSyncHumid200300S base class) but the NIGHTLIGHT feature flag is absent. Multi-firmware auto-mode try-fallback pattern applies (same as LV600S / pyvesync PR #505 risk for EU SKUs LUH-D301S-WEU/-KEUR). Other scenarios synthesized. |
