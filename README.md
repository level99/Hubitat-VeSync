...
| **Levoit Sprout Humidifier** *(v2.3 preview)* | `LEH-B381S-WUS`, `LEH-B381S-WEU` | Switch, MistLevel (**1-2** — 2-level hardware), TargetHumidity (30-80), Mode (auto/sleep/manual), DryingMode, ChildLock, Display, Humidity sensor, Temperature. `WEU` adds nightlight with brightness + color temperature (2000-3500K). pyvesync class `VeSyncSproutHumid`; auto mode wire value = `"autoPro"`. |
...
For upcoming devices beyond v2.10: [`ROADMAP.md`](ROADMAP.md).
...
- **HPM Update fails with `HTTP 500 Server Error` or `NullPointerException` during upgrade** — usually a transient `raw.githubusercontent.com` CDN hiccup (driver source is hosted on GitHub Raw, which intermittently returns 500/503 during GitHub's occasional rough days). The `NullPointerException` that often follows is HPM's own rollback path tripping on the failed download — not a bug in this package. Recovery: HPM → **Repair** → select "Levoit Air Purifiers, Humidifiers, and Fans" → click Next. Repair re-fetches the manifest + driver source and reconciles HPM's tracking. If Repair also fails, wait 5-10 min (GitHub CDN transients are usually short) and retry; check [GitHub Status](https://www.githubstatus.com/) if it persists.
