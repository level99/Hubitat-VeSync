...
12. **Parallel external reviewer alongside QA — SHOULD, every round.** Each QA round, the orchestrator SHOULD dispatch ONE external CLI reviewer in parallel with the `vesync-driver-qa` agent: **`opencode-go/deepseek-v4-flash`** (PAID, ~$0.033/round — the $/finding-dominant member of the ship-gate pack at iteration scale). It's rounding-error on the Sonnet QA cost (~$0.50-1), and the distinct-family second voice catches what Sonnet misses (sibling-pattern gaps, doc-vs-code drift, vacuous-guard tests). This is the *iteration-tier* analogue of the ship gate's multi-family fan-out — one model, not the pack; per-round, not per-PR. **The free seats (`opencode/mimo-v2.5-free`, `opencode/big-pickle`) are RESERVED for the ship-gate pack, not used here** — they're concise/narrow on their own, and Flash's wider net beats them at iteration scale where $0-vs-$0.033 is noise.
    - **SHOULD, not MAY.** A MAY-framed rule lets the model rationalize skipping ("small change, skip the parallel reviewer this round") and it silently never runs. Skipping is allowed only as an *explicit* decision (doc-only change, late-round tiny delta, near monthly cap), surfaced to the user — never silent omission.
    - **It's a `Bash` invocation, NOT a Claude sub-agent** — no agent file, no `agentId`, no `SendMessage`; fresh dispatch each round. Reuse the SAME shared external-reviewer prompt template as `/final-review` Step 4c — don't author a second prompt. Pre-stage the diff to a workdir-relative file and run `--dir .` per the ship-gate OpenCode notes (the `--dir .` policy auto-rejects `/tmp`, so the model must read a workdir diff file). External-CLI invocations CANNOT live in `.claude/agents/` (the `Agent({subagent_type})` loader won't find a Bash shim) — dispatch inline from the orchestrator.
    - **Reconcile QA-Sonnet vs parallel-external each round:** both clean → tester; Sonnet APPROVE + external flags BLOCKING/WARN → verify against source (hallucination risk, esp. *doc-drift* claims — open the file and read the line) and if real treat as a QA finding; Sonnet ISSUES + external clean → trust Sonnet (it knows project conventions + sibling parity the external sometimes misses); both flag different → consolidated send-back after per-finding verification; both flag the same → high-confidence.
    - **Cost ceiling:** Sonnet QA + external ≈ $0.55-1.05/round. If the external ever exceeds the QA cost, something's wrong (wrong model tier / runaway prompt size) — escalate the architecture, don't just absorb it.
...
| **Ship gate** | `/final-review` skill | ONCE before opening a PR or flipping draft → ready-for-review | ~$8-10 per run (Haiku pre-flight + 6 specialized Claude sub-agents in parallel fan-out + external different-family reviewers — OpenAI Codex CLI + the OpenCode standard 5-pack: `mimo-v2.5-free`+`big-pickle` FREE, `deepseek-v4-flash`+`kimi-k2.6`+`deepseek-v4-pro` paid Go ≈ $0.85) + 1 ChatGPT Plus message. Gemini deferred (cost-uncompetitive on its required paid tier) | ~100% per run against this fork's review style |
...
| OpenCode standard 5-pack | FULL independent second-opinion review from several distinct model families at once. **Members:** `opencode/mimo-v2.5-free` (FREE) · `opencode/big-pickle` (FREE) · `opencode-go/deepseek-v4-flash` (PAID ~$0.033) · `opencode-go/kimi-k2.6` (PAID ~$0.50) · `opencode-go/deepseek-v4-pro` (PAID ~$0.31) ≈ **$0.85/run**. Two free seats anchor distinct-family signal at $0 if the paid Go account is rate-capped. Blind-spot benefit compounds — each catches BLOCKINGs the others miss. One `Bash` call PER MEMBER; runs `--dir .` scoped to the cwd reading the pre-staged `.review_diff_<short-sha>.txt` (the `--dir .` policy blocks `/tmp`); needs `rg` on PATH. Per-member auth-fail/cost varies — drop a member, keep the pack. Dispatch ALL 5 if the family gate is YES (no cherry-picking). Consumes the SAME shared prompt. |
