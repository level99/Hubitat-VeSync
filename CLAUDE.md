...
| **Ship gate** | `/final-review` skill | ONCE before opening a PR or flipping draft → ready-for-review | ~$8-10 per run (Haiku pre-flight + 6 specialized Claude sub-agents in parallel fan-out + two different-family external reviewers — OpenAI Codex CLI + Google Gemini CLI) + 1 ChatGPT Plus message + 1 Gemini free-tier run | ~100% per run against this fork's review style |
