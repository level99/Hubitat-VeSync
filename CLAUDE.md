...
12. **Parallel external Flash-pair alongside QA — SHOULD, every round.** Each QA round, the orchestrator SHOULD dispatch a **Flash-tier pair** in parallel with the `vesync-driver-qa` agent: **`opencode-go/deepseek-v4-flash`** (PAID, ~$0.033/round, 1M context — the primary reliable voice, $/finding-dominant) **plus `opencode/deepseek-v4-flash-free`** (FREE, $0, 200K context — bonus voice). The same model architecture under two context budgets surfaces different findings (bake-off 2026-06-05: ~4 unique substantive findings each). Both run concurrently with Sonnet QA in the same Bash beat; combined they're rounding-error on the Sonnet QA cost (~$0.50-1), and the distinct-family voices catch what Sonnet misses (sibling-pattern gaps, doc-vs-code drift, vacuous-guard tests). This is the *iteration-tier* analogue of the ship gate's fan-out — the Flash pair, not the full 6-pack; per-round, not per-PR. **The slow free seats (`opencode/mimo-v2.5-free` ≈ 2 min, `opencode/big-pickle` ≈ 3 min) are RESERVED for the ship-gate** — their 2-3 min/round latency doesn't pay at iteration cadence where every loop waits on the slowest reviewer; Flash returns in ~30-60s.
    ...
    - **It's a `Bash` invocation, NOT a Claude sub-agent** — no agent file, no `agentId`, no `SendMessage`; fresh dispatch each round. Reuse the SAME shared external-reviewer prompt template as `/final-review` Step 4c — don't author a second prompt. Invocation: `timeout 1200 opencode run --model <m> --dir . "$(cat <prompt>)" < /dev/null > <out> 2>&1` — **two MANDATORY guards**: (1) `< /dev/null` — without it `opencode run` hangs on stdin under the Claude Code Bash tool on Windows/Git-Bash (bootstraps, never calls the model, 0 bytes both streams); (2) the command-level `timeout <sec>` wrapper (~1200s/20 min for Flash) — under Git-Bash `opencode run` frequently leaves a **wrapper process that never exits AFTER the model has already written its full output**, so a backgrounded call blocks forever and its completion notification never fires (lived 2026-06-06: a free-audit sweep hung ~9h with every output already on disk + 3 orphan bash wrappers). The Bash-tool `timeout:` param does NOT save you — a detached wrapper outlives it; the ceiling MUST be the per-call `timeout` in the command (output lands before it fires, so nothing is lost). `--dangerously-skip-permissions` is a no-op Claude-Code flag OpenCode silently drops — don't add it. Pre-stage the diff to a workdir-relative file (the `--dir .` policy auto-rejects `/tmp`) AND make the prompt positively direct the model to read that staged file + explicitly forbid `git diff`/scratch-writes (the `DO NOT run git diff yourself` / `DO NOT write any scratch files` lines) — staging alone is insufficient, some models still reach for `git diff > /tmp/x` and abandon the review (see `/final-review` Step 4a). External-CLI invocations CANNOT live in `.claude/agents/` (the `Agent({subagent_type})` loader won't find a Bash shim) — dispatch inline from the orchestrator. **Co-launch the `vesync-driver-qa` Agent AND both Flash Bash calls in ONE message** (multiple tool blocks, one response) — a sub-agent dispatched alone holds the foreground turn and the Flash launches don't fire until it's force-backgrounded (the "parallel" pair sits idle); they have no dependency once the diff is staged, so fire all three together.
    - **Reconcile across THREE voices (Sonnet QA + paid Flash + free Flash) each round:** all three clean → tester; Sonnet APPROVE + either Flash flags BLOCKING/WARN → verify against source (hallucination risk, esp. *doc-drift* claims — open the file and read the line) and if real treat as a QA finding; Sonnet ISSUES + both Flashes clean → trust Sonnet (it knows project conventions + sibling parity the externals sometimes miss); either Flash 429s/auth-fails → drop that voice silently for the round, proceed with the rest; voices flag the same thing → high-confidence. **Free-Flash no-show signature:** a free voice that returns a banner-only stub (~tens of bytes, no findings, no error text) has almost certainly hit the OpenCode **HARD free cap** (`FreeUsageLimitError`) — the free tier swallows the provider 429 and retries internally to timeout, so it *hangs* rather than erroring cleanly (the `timeout` wrapper on the dispatch bounds it; the voice then benignly drops). This is expected, not a bug to debug — the hard cap resets in hours. It is NOT worth a per-round pre-probe at iteration cadence (the timeout wrapper + silent drop already handle it); the cheap DEBUG cap-detector (`--log-level DEBUG … grep FreeUsageLimitError`) belongs only to a free-*only* sweep pre-flight (see the `/free-audit` skill Step 2.5), not the per-round Flash pair.
...
| **Ship gate** | `/final-review` skill | ONCE before opening a PR or flipping draft → ready-for-review | ~$8-10 per run (Haiku pre-flight + 6 specialized Claude sub-agents in parallel fan-out + external different-family reviewers — OpenAI Codex CLI + the OpenCode standard 6-pack: `mimo-v2.5-free`+`big-pickle`+`deepseek-v4-flash-free` FREE, `deepseek-v4-flash`+`kimi-k2.6`+`deepseek-v4-pro` paid Go ≈ $0.85) + 1 ChatGPT Plus message. Gemini deferred (cost-uncompetitive on its required paid tier) | ~100% per run against this fork's review style |
...
Reviewers that the skill fans out to: the 6 Claude sub-agents defined under `.claude/agents/vesync-driver-qa-*.md` — `coverage` (Opus, `effort: xhigh`), `platform` (Sonnet), `protocol` (Sonnet), `adversarial` (Opus, `effort: max`), `design` (Sonnet, `effort: max` — the synthesis lens), `operator` (Sonnet) — plus `preflight` (Haiku) as the sequential gate, plus **two external families** orchestrated by the skill as parallel `Bash` calls (NOT Claude sub-agents): the **OpenAI Codex CLI** and the **OpenCode standard 6-pack** (one family, 6 model siblings dispatched in one beat: `mimo-v2.5-free` + `big-pickle` + `deepseek-v4-flash-free` FREE, `deepseek-v4-flash` + `kimi-k2.6` + `deepseek-v4-pro` paid Go ≈ $0.85/run). All consume one shared prompt and review the FULL change independently — their highest-value catches are independent findings, not gap-fills; the blind-spot benefit compounds across families. Each is independently optional and gated by its own Step-1b flag: absent/unauth/usage-blocked → the skill skips it (per-member for the pack) and runs with the rest — graceful, per-reviewer degrade. **Google Gemini is deferred** — its required paid Google AI/Vertex tier is cost-uncompetitive vs the OpenCode pack at ship-gate scale; recipe preserved documented-but-dormant in the handoff doc.
...
| OpenCode standard 6-pack | FULL independent second-opinion review from several distinct model families at once. **Members:** `opencode/mimo-v2.5-free` (FREE) · `opencode/big-pickle` (FREE) · `opencode/deepseek-v4-flash-free` (FREE) · `opencode-go/deepseek-v4-flash` (PAID ~$0.033) · `opencode-go/kimi-k2.6` (PAID ~$0.50) · `opencode-go/deepseek-v4-pro` (PAID ~$0.31) ≈ **$0.85/run**. Three free seats anchor distinct-family signal at $0 if the paid Go account is rate-capped. Blind-spot benefit compounds — each catches BLOCKINGs the others miss. One `Bash` call PER MEMBER; runs `--dir .` scoped to the cwd reading the pre-staged `.review_diff_<short-sha>.txt` (the `--dir .` policy blocks `/tmp`); needs `rg` on PATH; **stdin from `/dev/null`** (mandatory — without it `opencode run` hangs on stdin under the Bash tool). Per-member auth-fail/cost varies — drop a member, keep the pack. Dispatch ALL 6 if the family gate is YES (no cherry-picking). Consumes the SAME shared prompt. |
...
22. HTTP-error path log spam during network outages — `sendBypassRequest()` previously logged `logError` every poll cycle for every child when the hub lost internet (DNS failure, connection timeout, etc.) — e.g., 6 children × 1 cycle/min = 6 ERROR lines/min for the duration of the outage. Distinct from BP21 (which addresses empty-API-result path); this is the network-layer error path (`UnknownHostException`, `SocketTimeoutException`, `ConnectException`, `NoRouteToHostException`). Fix: classify exception by class name (cause-chain walk, depth 10, handles sandbox/proxy wrappers); on first network-class exception log one-time WARN and set `state.networkUnreachableSince` + `state.lastNetworkWarnHourBucket` (both parent-level — network outages affect all children identically); subsequent errors are DEBUG-only; `emitNetworkWarnIfDue()` re-surfaces a WARN once per epoch-hour bucket while still down (v2.9 hardening: keyed off `state.lastNetworkWarnHourBucket = now()/3600000` rather than a `now()-lastWarnAt` delta, so concurrent poll cycles in the same hour can't re-fire it in 1-min bursts; the recovery probe likewise uses a 5-min `state.lastNetworkProbeBucket`); recovery detected on first successful `httpPost()` completion → INFO with elapsed duration + all state fields cleared. v2.9 also added child-side dedup: each child downgrades its OWN network-class write-fail log (ERROR+recordError) to a single DEBUG during a known outage, via `reportWriteError()`/`httpOk()`/`reportWriteFailure()` in LevoitChildBase consulting `parent.isNetworkUnreachable()` — so a child re-driven by a rule during an outage no longer emits per-retrigger ERROR spam on top of the parent's BP22 WARN. Dual circuit-breaker: (1) top-level at `updateDevices()` entry skips the entire poll cycle during a known outage — prevents the parallel-child stall where all children dispatch `httpPost` near-simultaneously before the first failure can set state, causing all to stall for full connect-timeout; (2) per-call in `sendBypassRequest()` guards command-triggered (non-polling) calls. Both probe every 5 minutes for recovery. Coordination: top-level breaker sets `state.networkProbeInFlight = true` when firing a recovery probe, cleared in a `try/finally` at end of `updateDevices()`; per-call breaker checks `!state.networkProbeInFlight` so the probe cycle's children are not re-blocked. Covers both JDK and Apache HttpClient (8 exception classes total). Non-network exceptions keep the existing `logError` path. See `vesync-driver-qa.md` BP22 entry for full design.
