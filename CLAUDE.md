...
12. **Parallel external reviewer alongside QA — SHOULD, every round.** Each QA round, the orchestrator SHOULD dispatch ONE external CLI reviewer in parallel with the `vesync-driver-qa` agent: the **OpenCode cheap-tier model** (the $/finding-dominant member of the ship-gate pack). At ~$0.03/round it's rounding-error on the Sonnet QA cost (~$0.50-1), and the distinct-family second voice catches what Sonnet misses (sibling-pattern gaps, doc-vs-code drift, vacuous-guard tests). This is the *iteration-tier* analogue of the ship gate's multi-family fan-out — one model, not the pack; per-round, not per-PR.
...
| OpenCode pack | FULL independent second-opinion review from several distinct model families at once (one router → N siblings: free-curated + paid-cheap + paid-premium tiers). Blind-spot benefit compounds — each member catches BLOCKINGs the others miss. One `Bash` call PER MEMBER; runs `--dir .` scoped to the cwd reading the pre-staged `.review_diff_<short-sha>.txt` (the `--dir .` policy blocks `/tmp`); needs `rg` on PATH. Per-member auth-fail/cost varies — drop a member, keep the pack. Dispatch ALL members if the family gate is YES (no cherry-picking). Consumes the SAME shared prompt. |
