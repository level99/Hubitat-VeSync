...
| `docs/BUG-PATTERNS.md` | Canonical bug-pattern catalog (BP1–BP29): symptom, root cause, fix scope, canonical fix, lint rule, regression coverage. Single source of truth — everything else references it. | Trigger phrases: *"bug pattern"*, *"BP&lt;N&gt;"*, *"BP1"*…*"BP29"*, fixing or adding a driver bug, a finding citing a BP number, designing a lint rule for a pattern |
...
12. **Parallel external Flash-pair alongside QA — SHOULD, every round.** Each QA round, the orchestrator SHOULD dispatch a **Flash-tier pair** in parallel with the `vesync-driver-qa` agent, **via the non-agentic direct client** `/z/claude/bin/opencode_go_qa_client.py` (handoff §5b-pre, mandatory 2026-06-13 — NOT `opencode run`): **`deepseek-v4-flash`** (PAID Go, ~$0.033/round, 1M context, default `/zen/go/v1` base — the primary reliable voice, $/finding-dominant) **plus `deepseek-v4-flash-free`** (FREE, $0, 200K context, `--base https://opencode.ai/zen/v1` — bonus voice). The same model architecture under two context budgets surfaces different findings (bake-off 2026-06-05: ~4 unique substantive findings each). Both run concurrently with Sonnet QA in the same Bash beat; combined they're rounding-error on the Sonnet QA cost (~$0.50-1), and the distinct-family voices catch what Sonnet misses (sibling-pattern gaps, doc-vs-code drift, vacuous-guard tests). This is the *iteration-tier* analogue of the ship gate's fan-out — the Flash pair, not the full 5-pack; per-round, not per-PR. **The slower free seats (`mimo-v2.5-free`, `nemotron-3-ultra-free`) are RESERVED for the ship-gate** — Flash returns fastest, which matters at iteration cadence where every loop waits on the slowest reviewer. **Fail-safe: the client is maintainer-local (in Z:/claude/bin, not the repo)** — a 3rd-party contributor without it just runs Sonnet QA alone; gate on `test -f /z/claude/bin/opencode_go_qa_client.py` and skip the pair silently if absent.
    - **SHOULD, not MAY.** A MAY-framed rule lets the model rationalize skipping ("small change, skip the parallel pair this round") and it silently never runs. Skipping is allowed only as an *explicit* decision (doc-only change, late-round tiny delta, paid Flash unavailable / near monthly cap), surfaced to the user — never silent omission. The FREE Flash is "best-effort dispatch" — never block the loop waiting for it to recover from a 429.
    - **It's a `Bash` invocation, NOT a Claude sub-agent** — no agent file, no `agentId`, no `SendMessage`; fresh dispatch each round. Reuse the SAME shared external-reviewer prompt template as `/final-review` Step 4c — don't author a second prompt. Invocation (direct client, ONE HTTPS POST — none of the old `opencode run` plumbing): `uv run --no-project --python 3.12 /z/claude/bin/opencode_go_qa_client.py -m <model> [--base https://opencode.ai/zen/v1 for the FREE seat] --user-file <prompt> -a diff=<staged.diff> -a <changed-file>=<path>… -a bugpatterns=docs/BUG-PATTERNS.md > /c/tmp/qa/<out> 2>&1`. **Context is the caller's job** — stage the diff + attach the FULL changed files + convention docs (a diff-only review false-positives); the client reads the attachments itself, their bytes never enter this session. **NO `< /dev/null`, NO `timeout 2700`/exit-wedge, NO `OPENCODE_DB`, NO `--dir`/ANSI-strip** — that entire agentic gotcha pile is retired; the client exits cleanly (its own `--timeout`, default 1200s, is the ceiling) and surfaces real HTTP errors (`FreeUsageLimitError`, 404, auth) as clean text instead of hanging. Write output to `/c/tmp/qa` (off the repo / OneDrive). External-CLI invocations CANNOT live in `.claude/agents/` — dispatch inline from the orchestrator. **Co-launch the `vesync-driver-qa` Agent AND both Flash Bash calls in ONE message** (multiple tool blocks, one response) — a sub-agent dispatched alone holds the foreground turn and the Flash launches don't fire until it's force-backgrounded (the "parallel" pair sits idle); they have no dependency once the diff is staged, so fire all three together.
    - **Reconcile across THREE voices (Sonnet QA + paid Flash + free Flash) each round:** all three clean → tester; Sonnet APPROVE + either Flash flags BLOCKING/WARN → verify against source (hallucination risk, esp. *doc-drift* claims — open the file and read the line) and if real treat as a QA finding; Sonnet ISSUES + both Flashes clean → trust Sonnet (it knows project conventions + sibling parity the externals sometimes miss); either Flash errors/auth-fails → drop that voice silently for the round, proceed with the rest; voices flag the same thing → high-confidence. **Free-Flash cap signature:** with the direct client the free daily cap surfaces as a **clean `FreeUsageLimitError`** in that seat's output (no silent hang — the client returns the real error body), so just scan for it and drop the seat for the round (resets in hours; the paid Flash draws a separate Go quota and is unaffected). No DEBUG cap-detector probe needed — that was an `opencode run`-only workaround.
    - **Cost ceiling:** Sonnet QA + Flash pair ≈ $0.55-1.05/round (paid Flash ~$0.033 + free Flash $0). If the paid external ever exceeds the QA cost, something's wrong (wrong model tier / runaway prompt size) — escalate the architecture, don't just absorb it.
...
| **Ship gate** | `/final-review` skill | ONCE before opening a PR or flipping draft → ready-for-review | ~$8-10 per run (Haiku pre-flight + 6 specialized Claude sub-agents in parallel fan-out + external different-family reviewers — OpenAI Codex CLI + the OpenCode standard 5-pack **via the direct client** `/z/claude/bin/opencode_go_qa_client.py`: `mimo-v2.5-free`+`deepseek-v4-flash-free`+`nemotron-3-ultra-free` FREE, `deepseek-v4-flash`+`deepseek-v4-pro` paid Go ≈ $0.34) + 1 ChatGPT Plus message. The OpenCode client is maintainer-local (not in-repo) → gated, fail-safe for 3rd parties. Gemini deferred (cost-uncompetitive on its required paid tier) | ~100% per run against this fork's review style |
...
Reviewers that the skill fans out to: the 6 Claude sub-agents defined under `.claude/agents/vesync-driver-qa-*.md` — `coverage` (Opus, `effort: xhigh`), `platform` (Sonnet), `protocol` (Sonnet), `adversarial` (Opus, `effort: max`), `design` (Sonnet, `effort: max` — the synthesis lens), `operator` (Sonnet) — plus `preflight` (Haiku) as the sequential gate, plus **two external families** orchestrated by the skill as parallel `Bash` calls (NOT Claude sub-agents): the **OpenAI Codex CLI** and the **OpenCode standard 5-pack via the non-agentic direct client** `/z/claude/bin/opencode_go_qa_client.py` (one HTTPS POST per model — NOT `opencode run`; 5 model siblings dispatched in one beat: `mimo-v2.5-free` + `deepseek-v4-flash-free` + `nemotron-3-ultra-free` FREE, `deepseek-v4-flash` + `deepseek-v4-pro` paid Go ≈ $0.34/run). All consume one shared prompt and review the FULL change independently — their highest-value catches are independent findings, not gap-fills; the blind-spot benefit compounds across families. Each is independently optional and gated by its own Step-1b flag: absent/unauth/usage-blocked → the skill skips it (per-member for the pack) and runs with the rest — graceful, per-reviewer degrade. **The OpenCode direct client is maintainer-local (in Z:/claude/bin, not the repo)**, so for a 3rd-party contributor the whole pack gates off and the Claude lenses (+ Codex if present) carry the review. **Google Gemini is deferred** — its required paid Google AI/Vertex tier is cost-uncompetitive vs the OpenCode pack at ship-gate scale; recipe preserved documented-but-dormant in the handoff doc.
...
| OpenCode standard 5-pack (direct client) | FULL independent second-opinion review from several distinct model families at once, via the non-agentic direct client `/z/claude/bin/opencode_go_qa_client.py` (ONE HTTPS POST per model — NOT `opencode run`). **Members (bare ids):** `mimo-v2.5-free` (FREE, Xiaomi) · `deepseek-v4-flash-free` (FREE, DeepSeek @200K) · `nemotron-3-ultra-free` (FREE, Nvidia — most precise free voice) · `deepseek-v4-flash` (PAID Go ~$0.033, recall) · `deepseek-v4-pro` (PAID Go ~$0.31, judgment) ≈ **$0.34/run**. 3 FREE distinct families anchor signal at $0 if the Go account is rate-capped; the 2 PAID Go seats are the throttle-proof fallback. Blind-spot benefit compounds. One `Bash` call PER MEMBER; the client reads the ATTACHED diff + full changed files + convention docs itself (their bytes never enter this session); FREE seats route `--base …/zen/v1`, PAID Go use the default `/zen/go/v1` quota. NONE of the old `opencode run` plumbing (no `--dir`/`/dev/null`/`timeout`/`OPENCODE_DB`/`rg`/ANSI/side-write/step-cap). **Maintainer-local client → absent for 3rd parties → whole pack gates off (fail-safe).** Per-member free-cap/retire varies — drop a member, keep the pack. Dispatch ALL 5 if the gate is YES (no cherry-picking). Consumes the SAME shared prompt. |
| ~~Gemini CLI (Google)~~ | **DEFERRED** — not in the active fan-out; required paid Google AI/Vertex tier is cost-uncompetitive vs the OpenCode pack at ship-gate scale. Recipe documented-but-dormant in the handoff doc (`--yolo`-in-worktree, `rg`-on-PATH, paid-tier lessons preserved for a future re-evaluation). |
...
3. Revert / delete / replace the fix with a minimal no-op mutation (e.g., replace a call with `pass` / delete the assertion (Python lint-rule tests), or comment out / replace with `return` (Groovy Spock specs)). Run the guard again — it MUST fail. Record the verbatim failure block. **When the mutation touches ONLY driver/lib `.groovy` (no `src/test/` change), run the Spock leg with `./gradlew test --rerun-tasks`** — driver/lib files are read by the harness at runtime, not Gradle compile inputs, so a bare `./gradlew test` can judge the task `UP-TO-DATE`, skip it, and reuse the pre-mutation results — a **false GREEN that masks a vacuous guard in the failing direction**. (A spec/fixture edit makes the task non-`UP-TO-DATE` on its own; the bare command is safe then. Also: when reverting an untracked/newly-staged file, restore from a saved `cp`, not `git checkout` — the latter pulls the stale staged blob.)
...
The canonical bug-pattern catalog (BP1–BP29 — symptom, root cause, fix scope, canonical fix, lint rule, regression coverage) lives in **`docs/BUG-PATTERNS.md`**, the single source of truth. CONTRIBUTING.md, the QA agent definitions, and the external-reviewer prompts all reference it rather than duplicating it. Read it on-demand when bug-pattern work comes up — a diff touching a `set*`/`doSet*`/`update`/`applyStatus` path, a finding citing `BP#N`, or adding a new pattern. When recognizing a pattern in a diff, name it explicitly — *"Bug Pattern #1 — missing 2-arg signature."* — and apply the catalog's canonical fix.
