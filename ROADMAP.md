...
- **Coverage WARN-defers from v2.7 final-review.** Two regression-guard specs deferred from v2.7's coverage sub-agent: (1) `traceSeq` modulo-99999 wraparound regression-guard — verifies the modulo clamp doesn't silently wrap to 0 at the 100k mark; (2) `acctId` fallback-branch spec — verifies the empty-stage1-accountID fallback path in the two-stage auth exchange. Both are belt-and-suspenders coverage; no behavior gap, both code paths empirically exercised in the live prod-hub verification.
