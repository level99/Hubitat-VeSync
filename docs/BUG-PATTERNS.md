...
**Regression coverage:** every BP6-bearing driver ships a from-off Spock guard asserting the active level emits 0/"off" when the device is off WHILE the API still reports a retained nonzero level (a fixture with `enabled:false` + nonzero `mist_virtual_level`/`warm_level`/`level`/`fanSpeedLevel`). Note: a from-off test whose fixture already has the level at 0 is VACUOUS — it passes with or without the clamp; the retained-nonzero fixture is what makes the guard real. The warm-`enabled`-fallback edge needs a fixture that OMITS `warm_level` and sets `warm_enabled:true`. Covered drivers (each with a non-vacuous retained-nonzero from-off guard): LV600S, LV600S Hub Connect, OasisMist 450S, Classic 200S/300S, Dual 200S, Superior 6000S, OasisMist 1000S, Sprout Humidifier (mist/warm); Core 200S/300S/400S/600S (speed); EverestAir, Sprout Air (fanSpeed). Note on the purifier fanSpeed cases (EverestAir, Sprout Air): the `fanSpeedLevel == 255` sentinel maps to 0 BEFORE `clampOffLevel`, so the 255 from-off test is vacuous w.r.t. the clamp — the non-vacuous guard for those two exercises the `manualSpeedLevel` fallback branch (`fanSpeedLevel` absent + retained `manualSpeedLevel`), where only `clampOffLevel` forces 0. The shared `clampOffLevel` helper is unit-tested in `LevoitChildBaseLibSpec`.
...
**Failure-path robustness (v2.10 /code-review hardening):**
- **A1 — dedup placement before early-return delegation.** A mode/speed setter whose early-return branch delegates to ANOTHER setter (EverestAir/SproutAir `setMode("manual")` → `setFanSpeed`; Vital `setSpeed("sleep")` → `setMode`) must run `isDuplicateWrite(slot, value)` BEFORE that delegation, so the slot records the NEW effective value. Otherwise `auto → manual → auto` within the window leaves the slot stale at `"auto"` and the 3rd write is falsely suppressed, stranding the device in manual. Reference shape: Vital `setMode` (dedup precedes the manual branch).
- **B1 — record-on-proceed, clear-on-failure.** `isDuplicateWrite` records the slot when it PROCEEDS (so A1's delegation paths record the new value). When the subsequent cloud write FAILS, the write-failure branch calls `clearDuplicateWrite(slot)` to undo the record, so an immediate same-value retry is NOT falsely suppressed. Identical SUCCESSFUL writes still coalesce. `clearDuplicateWrite` MUST use String (not GString) `state` keys — a GString key does not match `isDuplicateWrite`'s String-keyed entry and would silently fail to clear (caught by the both-ways spec). (Core `setSpeed` is the one deduped setter without a clean single failure branch — it routes through `handleSpeed`/`handleMode`, which report via `checkHttpResponse` internally; its failed-write retry is bounded by the 2 s window only.)
- **B2 — clear the power-on window on a FAILED `on()`.** A failed power-on must not hold the window open for `POWER_ON_WINDOW_MS` and suppress an immediate retry; every `on()` clears the window in its power-on-write-failure branch. Core `on()` was restructured to gate the speed/mode burst on `handlePower(true)` success (it previously proceeded unconditionally).
- **B3 — cancel the safety timer on clear.** `clearPowerOnWindow()` and `beginPowerOnWindow()` both `unschedule("clearPowerOnWindow")`, so an `on → off → on` sequence cannot leave an orphan `runInMillis` timer that closes the next window early. All three (A1/B1/B2) plus B3 ship both-ways-proven Spock guards.
