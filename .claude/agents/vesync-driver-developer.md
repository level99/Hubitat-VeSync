...
6. **Phase 5 standard plumbing — REQUIRED for every new child driver (enforced by lint RULE28).** Add at the top of the driver source: `#include level99.LevoitDiagnostics`. In the `metadata { definition(...) { ... } }` block, add `command "captureDiagnostics"` and `attribute "diagnostics", "string"`. Where the driver calls `log.error(...)`, add a parallel `recordError(message, [site:"<methodName>", ...])` call. The library exports `recordError`, `captureDiagnostics`, and ring-buffer state shared across all drivers — these are not optional. Lint RULE28 will FAIL on any driver missing the three plumbing pieces. See any v2.4+ child driver (e.g. `LevoitVital200S.groovy`) for the canonical pattern.
   ...
   **6a. When adding a NEW Hubitat library file (rare — most contributions don't need this):** library files MUST use `// line comments` for the file-scope documentation header, NOT `/* ... */` block comments. Hubitat's library parser silently rejects library source containing a `/* */` doc block at the top of the file with `{"success":false,"message":"Internal error"}` from `POST /library/saveOrUpdateJson` — confirmed reproduces on FW 2.4.4.156 AND 2.5.0.126. HPM uses the same endpoint, so a library that uses `/* */` cannot be installed by end users. Single-line `// ...` comments work cleanly. The MIT/copyright header (one `/* */` block at the very top before any other content) is fine; the bug triggers on the SECOND `/* */` block. javadoc `/* */` blocks INSIDE method bodies are also fine. Lint RULE29 (`tests/lint_rules/library_no_top_block_comment.py`) FAILs on any library file violating this. See `Drivers/Levoit/LevoitDiagnosticsLib.groovy` for the canonical example — the in-source NOTE block explains the workaround. This is Bug Pattern #20 in the catalog.
