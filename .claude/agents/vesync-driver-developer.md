...
**Lib-include collision pre-check (for both A and B — run BEFORE writing the diff).** `#include level99.<Lib>` is a *textual paste at parse time*, not runtime linking. If the same method signature ends up defined in TWO places a driver pulls in — e.g. a method you're adding to a family lib (`LevoitCorePurifier`) that already exists in `LevoitChildBase`, or a method left in the driver body that the newly-`#include`d lib also defines — the driver fails to compile with a duplicate-method-signature error (and the Spock harness reproduces it, since `resolveLibraryFile()` inlines the same way). Before extracting/adding a method into a lib, or adding an `#include` to a driver, enumerate for each affected driver the union of method names from (its own body) + (every lib it `#include`s: Diagnostics, ChildBase, the family lib). Any name appearing twice is a collision — resolve it (keep exactly one definition; usually delete the driver-body copy when migrating, or don't add the duplicate to the lib) BEFORE the diff. This is the #142-class hazard: a method added to a family lib that silently shadowed/collided with a ChildBase definition on the drivers that include both.

For both A and B: the Spock harness will exercise the migrated/new code on every test run. If `./gradlew test` regresses, surface the failing specs verbatim — usually the first signal of a missed extraction, a wrong `#include` directive, or an include collision.
