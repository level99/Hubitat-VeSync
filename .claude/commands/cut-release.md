...
- This procedure does NOT push to remote, does NOT open a PR, does NOT tag, does NOT update GitHub releases. Those happen across the steps above:
  ...
  * `gh release create` targets the release branch HEAD (e.g. `--target release/v<version>`); the resulting tag points at that commit. **Squash-merge is the expected merge method.** A squash-merge creates a NEW commit on main, so the tagged release-branch commit is NOT an ancestor of main afterward — but the tag stays valid for HPM regardless, because the two resolution paths are independent: HPM fetches the bundle by RELEASE TAG (`releases/download/v<version>/…`, attached to the release object — independent of main's history) and fetches the manifest + driver source by the `main` BRANCH (`raw.githubusercontent.com/.../main/…`, which carries the squashed v<version> content). Neither path needs the tagged commit to be reachable from main.
