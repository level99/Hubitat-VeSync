...
| **BP22 — HTTP-error log dedup during outages** | Convention | First network-class exception (UnknownHostException / SocketTimeoutException / ConnectException / NoRouteToHostException + Apache HttpClient equivalents) logs one-time WARN and sets `state.networkUnreachableSince` + `state.lastNetworkWarnHourBucket` (parent-level — outages affect all children identically). Subsequent errors are DEBUG-only. `emitNetworkWarnIfDue()` re-surfaces a WARN once per epoch-hour bucket while still down (v2.9: bucket form `now()/3600000`, not a `now()-lastWarnAt` delta — so concurrent poll cycles in the same hour can't re-fire it in 1-min bursts). Recovery on first successful httpPost → INFO with elapsed duration + state cleared. Dual circuit-breaker: top-level at `updateDevices()` skips entire poll cycle during a known outage; per-call in `sendBypassRequest()` guards command-triggered calls. Both probe every 5 minutes for recovery (5-min `state.lastNetworkProbeBucket`); coordinate via `state.networkProbeInFlight` flag. Cause-chain walk (depth 10) handles sandbox/proxy exception wrappers. v2.9 child-side dedup: children downgrade their OWN network-class write-fail logs (ERROR+recordError) to a single DEBUG during a known outage via `reportWriteError`/`httpOk`/`reportWriteFailure` consulting `parent.isNetworkUnreachable()`. |
