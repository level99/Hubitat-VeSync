...
11. **Apply BP24 auto-on-from-off discipline.** For each command method you declare, classify it per Bug Pattern #24's taxonomy (MUST-ON / SHOULD-ON / SKIP-OK / NO-ON — see `docs/BUG-PATTERNS.md` BP24 catalog entry for the full classification chart and sub-shape descriptions):
...
The canonical bug-pattern catalog lives in **`docs/BUG-PATTERNS.md`** (BP1–BP29: symptom, root cause, fix scope, canonical fix, lint rule, regression coverage) — the single source of truth. Lint rules and Spock specs cite these numbers in finding/test-name strings; when you see `BP9` in a finding, that's catalog entry #9 (driver-name change orphan). **Read `docs/BUG-PATTERNS.md` before writing a new driver** — BP1 (3-signature `update()`), BP4 (V2-API field names), and BP12 (pref-seed) in particular shape how every driver is structured.
