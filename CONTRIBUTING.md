...
| **Persistent poll schedule** | BP14 | Parent driver (`VeSyncIntegration.groovy`) must use `schedule()` — not recursive `runIn()` — for the poll cycle. `schedule()` survives hub reboots; `runIn()` does not. Enforced via `ensurePollWatchdog()` + `setupPollSchedule()` in `initialize()` and `updateDevices()`. Spec coverage: `VeSyncIntegrationSpec` I1–I8. |
